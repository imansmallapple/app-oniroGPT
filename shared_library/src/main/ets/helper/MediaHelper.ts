...
export class MediaHelper {
  ...
  public async convertToBase64() {
    try {
      ...
      await this.getPixelMap(); // Ensure PixelMap is created
      await this.pixelToBase64(); // Then perform Base64 conversion
      return this.base64
    } catch (err) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        imageSourceApi.createPixelMap({ editable: true }).then(pixelmap => {
          ...
          resolve(); // Return success after decoding is complete
